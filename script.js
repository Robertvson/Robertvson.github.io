...
const it = {
    ...
    eduCosa: 'Higher National Diploma in Technical Support and Cyber Security (Diploma Nazionale Superiore di Supporto Tecnico e Sicurezza Cyber',
...
const traduzioneFunc = () => document.querySelectorAll('[data-traduzione]').forEach(el => {
    const keyit = el.getAttribute('data-traduzione'); // Access element's attribute
    el.textContent = it[keyit]; // Update element's content
});

const translationFunc = () => document.querySelectorAll('[data-translation]').forEach(el => {
    const keyen = el.getAttribute('data-translation'); // Access element's attribute
    el.textContent = en[keyen]; // Update element's content
});

let x = 0

while(x > 0){
    if(document.getElementById('tradurre').onclick === true && x % 2 === 0){
        document.getElementById('tradurre').onclick = traduzioneFunc
    }else{
        document.getElementById('tradurre').onclick = translationFunc
    }
}